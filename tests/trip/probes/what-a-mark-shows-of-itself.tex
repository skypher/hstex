...
%
% Still open here: the text a mark expands into is read back from a frame
% the reference calls `<mark>' and shows as a token list, with a parameter
% character doubled; HSTeX calls it `<inserted text>' and shows it singly.
% And the reference traces the `#' it reads back as a command before
% refusing it, where HSTeX refuses it untraced.
