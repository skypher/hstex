...
%
% Found and left open: a \vcenter is hung on the axis when its box is made,
% where the reference hangs it when the atom is SET -- so the box a formula
% holds shows the axis already taken off, and in a script, where the axis is
% a different one, the box comes out a different size as well.
